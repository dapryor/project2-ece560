...
TCS_RETURN_T SD_Write(SD_DEV *dev, void *dat, DWORD sector)
{
    static WORD idx;
    static BYTE line;
		static TCS_RETURN_T write_return;
		static enum {S1,S2,S3,S4,S4a,S5} state = S1;

		// Query invalid? S1 - input accepted
    ...
    if(__SD_Send_Cmd(CMD24, sector * SD_BLK_SIZE)==0)/* S1*/ {
			...
			if((SPI_RW(0xFF) & 0x1F) != 0x05) {
				return(SD_REJECT); //Valid Return
			}}
			
			// Waits until finish of data programming with a timeout
			SPI_Timer_On(SD_IO_WRITE_TIMEOUT_WAIT); //S4
			do {
					{line = SPI_RW(0xFF);} // S4a
